...
<div class="container-fluid">
  <h1 class="h3 mb-2 text-gray-800">Pantallas</h1>
    ...
    <div class="card-body">
      <div class="table-responsive">
        <table class="table table-hover table-sm" id="dataTable" width="100%" cellspacing="0">
          ...
          <tbody>
            @foreach ($pantallas as $s)
            <tr>
              <td>{{ $s->id }}</td>
              <td><img src="{{ asset($s->getPhoto()) }}" class="rounded"  alt="" style="width: 40px;" width="60px"></td>
              <td>{{ $s->subdomain }}</td>
              <td><a href="{{ route('admin.pantalla.show',$s->id) }}">{{ $s->nombre }}</a></td>
              {{-- <td><small>{{ $s->sede->nombre }}</small></td> --}}
              {{-- <td>
                @if ($u->tipo_usuario == 1)
